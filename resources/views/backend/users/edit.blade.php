...
@push('site_title', trans('users.titles.edit'))
...
@section('content')
            ...
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">{{ trans('users.titles.edit') }}</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('backend.users.update', ['userId' => $user->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-12 mb-3 role-select">
                                <div class="form-group {{ $errors->has('role') ? 'has-error' : null }}">
                                    ...
                                    <select name="role" id="role" class="form-control {{ $errors->has('role') ? 'is-invalid' : null }}">
                                        ...
                                        <option value="{{ \App\Models\User::ROLE_ADMIN }}" {{ old('role', $user->role) == \App\Models\User::ROLE_ADMIN ? 'selected' : null }}>{{ trans('users.inputs.roles.options.' . \App\Models\User::ROLE_ADMIN) }}</option>
                                        <option value="{{ \App\Models\User::ROLE_AGENCY }}" {{ old('role', $user->role) == \App\Models\User::ROLE_AGENCY ? 'selected' : null }}>{{ trans('users.inputs.roles.options.' . \App\Models\User::ROLE_AGENCY) }}</option>
                                        <option value="{{ \App\Models\User::ROLE_STUDENT }}" {{ old('role', $user->role) == \App\Models\User::ROLE_STUDENT ? 'selected' : null }}>{{ trans('users.inputs.roles.options.' . \App\Models\User::ROLE_STUDENT) }}</option>
                                    </select>
                                    @if ($errors->has('role'))
                            ...
                            <div class="col-12 col-lg-6 mb-3 agencies {{ old('role', $user->role) != \App\Models\User::ROLE_AGENCY ? 'd-none' : null }}">
                            ...
                            <div class="col-12 col-lg-6 mb-3">
                                <div class="form-group {{ $errors->has('name') ? 'has-error' : null }}">
                                    ...
                                    <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : null }}" value="{{ old('name', $user->name) }}" />
                                    @if ($errors->has('name'))
                            ...
                            <div class="col-12 col-lg-6 mb-3">
                                <div class="form-group {{ $errors->has('email') ? 'has-error' : null }}">
                                    ...
                                    <input type="email" name="email" class="form-control {{ $errors->has('email') ? 'is-invalid' : null }}" value="{{ old('email', $user->email) }}" />
                                    @if ($errors->has('email'))
                            ...
                            <div class="col-12 col-lg-6 mb-3">
                                <div class="form-group {{ $errors->has('phone') ? 'has-error' : null }}">
                                    ...
                                    <input type="text" name="phone" class="form-control {{ $errors->has('phone') ? 'is-invalid' : null }}" value="{{ old('phone', $user->phone) }}" />
                                    @if ($errors->has('phone'))
                            ...
                            <div class="col-12 col-lg-6 mb-3">
                                <div class="form-group {{ $errors->has('avatar') ? 'has-error' : null }}">
                                    <label for="avatar">{{ trans('users.inputs.avatar') }}</label>
